...
def dashboard_window(window_size, main_file_directory, user_id_text_stored, user_password_text_stored):
    ...
    while event_flag:
        ...
        if Guide_flag:
            Guide_flag = False
            guide(window_size, main_file_directory, user_id_text_stored, user_password_text_stored)
            break
        for event in pygame.event.get():
        ...
        shop_na_text = display_text(dashboard_window, "N/A", uni_red, 413, 400, 15, "OCRAEXT")
        ...
        setting_na_text = display_text(dashboard_window, "N/A", uni_red, 640, 355, 15, "OCRAEXT")
        Logs_btn_text = display_text(dashboard_window, "Logs", black, 550, 390, 28, "ARLRDBD")
        logs_na_text = display_text(dashboard_window, "N/A", uni_red, 620, 400, 15, "OCRAEXT")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for column_count in range (1, 14):
                    ...
                    w = 40
                    row_count = 0
                    for card_num in range ((((column_count-1)*4)+1), ((column_count*4)+1)):
                        card = pygame.image.load(sevendeadlysins_index[card_num].card_image).convert_alpha()
